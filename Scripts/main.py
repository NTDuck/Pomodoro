...
def font_setup(folder: str, path: str, size: int) -> pygame.font.Font:
    return pygame.font.Font(os.path.join(f'{FOLDER}\{folder}', path), size)
...
FONT_NUM = font_setup('Fonts', 'JetBrainsMonoNL-Bold.ttf', 80)
FONT_CHR = font_setup('Fonts', 'RobotoCondensed-Regular.ttf', 15)
...
class SettingButton:
    def __init__(self, pos: tuple[int]):
        self.pos = pos
        self.img = pygame.Surface((27, 27), pygame.SRCALPHA)
...
class Number:
    ...
    def render(self, window: pygame.Surface, current_time: int):
        text = self.font.render(self.num_to_display(current_time), True, COL_MOCHA)
...
class Colon:
    ...
    def render(self, window: pygame.Surface):
        text = self.font.render(':', True, COL_MOCHA)
...
class App:
    def __init__(self):
        ...
        os.environ['SDL_VIDEO_WINDOW_POS'] = "%d,%d" % self.place_pos

        self.window = pygame.display.set_mode((WIDTH, HEIGHT), pygame.NOFRAME)

        self.total_time = 1500

        self.SysFunction = SysFunction(self.place_pos)
        del self.place_pos
        ...
        self.MainButton.current_time = self.total_time
    ...
    def handle_mouse(self, window: pygame.Surface, event: pygame.event.Event, mouse_pos: tuple[int], SysButtons: list[SysButton], SettingButton: SettingButton, MainButton: MainButton):
        if pygame.mouse.get_focused():
            ...
            if not self.isMenuToggled:
                ...
                self.SysFunction.onToggle(event, mouse_pos)
    ...
    def main(self):
        self.SysFunction.move_window(self.SysFunction.win_coords)
